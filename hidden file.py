...
if "concealData" in wenjian:
    ...
    while True:
        ...
        #主程序
        loginon("已进入菜单")
        cd = g.buttonbox("欢迎使用隐藏文件程序","版本3.1",choices=["隐藏/显示文件","访问Github","设置","查看隐藏记录","删除隐藏记录","更新日志","实验性功能","内测内容","退出"])
        if cd =="隐藏/显示文件":
            while True:
        ...
        if cd =="实验性功能":
            if GN1:
                ...
                if password == mima:
                    ...
                    if cd1 == "隐藏应用程序列表":
                        xuaze4 = g.buttonbox("请注意！本功能需要安装额外拓展，您是否需要安装？","提示",choices=["已安装继续打开","前往安装拓展"])
                        if xuaze4 == "已安装继续打开":
                            app()
                        else:
                            g.msgbox("下载完成之后请您将文件夹移动到程序根目录即可")
                            g.msgbox("提取码是:30iw","提示","开始下载")
                            os.system("start https://xiaozhux.lanzouv.com/b03jj283a")            
                else:
                    g.msgbox("授权码错误，如果忘记请联系管理员")
            else:
                g.msgbox("未检测到授权码文件，请您打开记录文件功能后尝试")
        if cd =="内测内容":
